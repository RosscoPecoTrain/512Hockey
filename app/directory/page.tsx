        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {profiles.map((profile) => (
            <Link key={profile.id} href={`/profile/${profile.id}`}>
              <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition cursor-pointer border border-gray-200 hover:border-[#4fc3f7]">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="w-12 h-12 bg-[#4fc3f7] rounded-full flex items-center justify-center text-white font-bold">
                ...
                {profile.leagues && profile.leagues.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-1">
                    {profile.leagues.slice(0, 2).map((league, idx) => (
                      <span
                        key={idx}
                        className="inline-block bg-[#4fc3f7] text-[#0a1628] text-xs px-2 py-1 rounded"
                      >
                        {league}
                      </span>
                    ))}
